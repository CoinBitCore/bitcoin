...
*Setup instructions for a Gitian build of Coinbit Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Coinbit
...
- [Building Coinbit Core](#building-coinbit-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-coinbit-host/README.md).
...
Clone the git repositories for coinbit and Gitian.
...
in the coinbit repository under 'Fetch and create inputs' to install sources which require
...
To build Coinbit Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the coinbit repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/coinbit/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/CoinBitCore/coinbit
...
For example:
```bash
URL=https://github.com/laanwj/coinbit.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit coinbit=${COMMIT} --url coinbit=${URL} ../coinbit/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit coinbit=${COMMIT} --url coinbit=${URL} ../coinbit/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit coinbit=${COMMIT} --url coinbit=${URL} ../coinbit/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the coinbit git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../coinbit/contrib/gitian-descriptors/*|sort|uniq )
...
CTBPATH=/some/root/path/coinbit
SIGPATH=/some/root/path/coinbit-detached-sigs

./bin/gbuild --url coinbit=${CTBPATH},signature=${SIGPATH} ../coinbit/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/coinbit-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/coinbit-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/coinbit-osx-build.assert
```
...
[coinbit-core/gitian.sigs](https://github.com/bitcoin-core/gitian.sigs/) repository, or if that's not possible create a pull
