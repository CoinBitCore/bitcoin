...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(COINBIT_IPC_PREFIX, Qt::CaseInsensitive)) // coinbit: URI
        {
            ...
            {
                CCoinbitAddress address(r.address.toStdString());
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(COINBIT_IPC_PREFIX, Qt::CaseInsensitive)) // coinbit: URI
    {
        ...
        else // normal URI
        {
            ...
            {
                CCoinbitAddress address(recipient.address.toStdString());
                if (!address.IsValid()) {
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            addresses.append(QString::fromStdString(CCoinbitAddress(dest).ToString()));
        }
