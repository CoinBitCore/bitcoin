...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CCoinbitAddress(CScriptID(redeemScript)).ToString();
...
bool CWallet::SetAddressBook(const CTxDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!strPurpose.empty() && !CWalletDB(*dbw).WritePurpose(CCoinbitAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(*dbw).WriteName(CCoinbitAddress(address).ToString(), strName);
}
...
{
    {
        ...
        std::string strAddress = CCoinbitAddress(address).ToString();
    ...
    CWalletDB(*dbw).ErasePurpose(CCoinbitAddress(address).ToString());
    return CWalletDB(*dbw).EraseName(CCoinbitAddress(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    return CWalletDB(*dbw).WriteDestData(CCoinbitAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    if (!mapAddressBook[dest].destdata.erase(key))
        return false;
    return CWalletDB(*dbw).EraseDestData(CCoinbitAddress(dest).ToString(), key);
}
